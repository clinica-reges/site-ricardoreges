...
export const metadata: Metadata = {
  title:
    'Cirurgia Robótica em Uro-Oncologia em Fortaleza — Próstata, Rim, Bexiga e Adrenal | Dr. Ricardo Reges',
  description:
    'Cirurgia robótica do câncer urológico em Fortaleza: prostatectomia, nefrectomia parcial, adrenalectomia, cistectomia e linfadenectomia. Avaliação completa com professor de urologia da UFC.',
...
// Mini-FAQ da seção "A uro-oncologia além da próstata" (renderizado dentro dela).
const FAQ_URO = [
  {
    q: 'Todas essas cirurgias são feitas em Fortaleza?',
    a: 'Sim — a prática robótica desta equipe ocorre no Hospital São Camilo, em Fortaleza, com agenda cirúrgica semanal.',
  },
  {
    q: 'Robótica é sempre a melhor via para esses tumores?',
    a: 'Não necessariamente — a via certa depende do tumor, da anatomia e do paciente. A avaliação apresenta as opções (robótica, laparoscópica, aberta ou endoscópica) com os prós e limites de cada uma para o seu caso.',
  },
  {
    q: 'Meu caso é raro. Isso muda algo?',
    a: 'Muda o peso da escolha da equipe. Tumores raros beneficiam-se de condução acadêmica: protocolos atualizados, discussão multidisciplinar e a experiência de quem ensina a especialidade.',
  },
];
      ...
      <JsonLd
        data={[
          procedurePageSchema({
            ...
            procedures: [
              'Prostatectomia radical robô-assistida (RARP)',
      ...
      <header className="page-head">
        <div className="wrap narrow">
          ...
          <p className="subtitle">
            ...
            <br />
            Uro-oncologia completa: próstata, rim, bexiga, adrenal e testículo
          </p>
      ...
      <div className="section">
        <div className="wrap prose">
          ...
          <p>
            ...
            e a mesma plataforma robótica, com os mesmos princípios de precisão e
            preservação, trata os tumores dos demais órgãos urológicos. Um resumo
            honesto de cada frente:
          </p>

          <h3>Rim — nefrectomia parcial e radical robótica</h3>
          <p>
            No tumor renal localizado, o objetivo moderno vai além de remover o
            câncer: sempre que a anatomia permite, a{' '}
            <strong>nefrectomia parcial</strong> retira apenas o tumor e{' '}
            <strong>preserva o restante do rim</strong> — e néfrons preservados
            hoje são função renal amanhã. A visão ampliada e a precisão dos
            instrumentos robóticos são particularmente valiosas nessa cirurgia de
            milímetros. Quando o tumor não permite preservação, a nefrectomia
            radical robótica mantém as vantagens da via minimamente invasiva. A
            formação uro-oncológica na UNICAMP incluiu cirurgia renal complexa,
            com publicação internacional sobre tumores renais com trombo de veia
            cava — o extremo de complexidade dessa doença<sup>7</sup>.
          </p>

          <h3>Adrenal — adrenalectomia robótica</h3>
          <p>
            A glândula adrenal fica em posição profunda e de acesso delicado —
            exatamente o cenário em que a via robótica mais se justifica: incisões
            pequenas onde a cirurgia aberta exigiria grandes, recuperação rápida
            num procedimento que costuma tratar tanto tumores quanto distúrbios
            hormonais. É uma das cirurgias robóticas com experiência acumulada
            nesta prática, em conjunto com as equipes de endocrinologia que
            conduzem o preparo hormonal.
          </p>

          <h3>
            Bexiga — da ressecção endoscópica à cistectomia radical robótica
          </h3>
          <p>
            O tumor de bexiga tem dois mundos: os superficiais, tratados por via
            endoscópica com ressecção e vigilância rigorosa; e os
            músculo-invasivos, cujo tratamento padrão pode exigir a{' '}
            <strong>cistectomia radical</strong> — uma das maiores cirurgias da
            urologia, em que a via robótica reduz sangramento e acelera a
            recuperação de um pós-operatório exigente. A decisão entre os caminhos
            (e as alternativas de preservação vesical, quando indicadas) é
            individualizada com a oncologia clínica.
          </p>

          {/* BLOCO EM ESPERA — publicar somente após os primeiros casos realizados
              (mesmo critério aplicado ao EchoLASER). NÃO renderizar até lá:

              Para tumores superficiais selecionados, a **ressecção em bloco com
              laser de alta potência** representa a evolução da ressecção
              convencional: o tumor é removido em peça única, com base e margens
              íntegras para o patologista — mais informação diagnóstica em um
              único procedimento. A técnica soma-se à plataforma de cirurgia a
              laser que esta prática utiliza desde 2012. */}

          <h3>Testículo — linfadenectomia retroperitoneal robótica</h3>
          <p>
            O câncer de testículo é raro — e a cirurgia dos seus linfonodos
            (linfadenectomia retroperitoneal), mais rara ainda: em nenhum lugar do
            mundo cirurgiões acumulam grandes casuísticas individuais dela. Por
            isso, a resposta honesta sobre o que importa aqui não é um número, e
            sim a <strong>fundação</strong>: a cirurgia segue os mesmos princípios
            oncológicos e a mesma plataforma robótica da prática semanal desta
            equipe, conduzida por professor de urologia habituado a formar
            cirurgiões e a navegar do caso simples ao complicado — a diferença que
            o prognóstico sente. O tratamento é sempre integrado à oncologia
            clínica, porque no testículo a cirurgia é parte de uma estratégia, não
