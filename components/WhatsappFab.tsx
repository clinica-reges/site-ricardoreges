...
export default function WhatsappFab() {
  ...
  useEffect(() => {
    ...
    if (!fab || typeof IntersectionObserver === 'undefined') return;

    const ctaRows = document.querySelectorAll('.cta-row');
    const alvos = [
      document.querySelector('.site-footer'),
      ctaRows.length ? ctaRows[ctaRows.length - 1] : null,
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        fab.dataset.oculto = visiveis.size ? 'true' : 'false';
      },
